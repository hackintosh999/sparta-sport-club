...
exports.MemoryBankSync = void 0;
const fs = __importStar(require("fs/promises"));
const fsCallback = __importStar(require("fs"));
...
class MemoryBankSync {
    constructor(workspacePath, mcpDataPath = '.agentMemory') {
...
**Note:** This memory was auto-created to demonstrate the system.`,
     ...
     */
    parseMarkdown(content, filename, config, agentName) {
        const memories = [];
        ...
        if (matches.length === 0) {
            ...
            if (contentWithoutFrontmatter.trim()) {
                memories.push(this.createMemory(filename.replace('.md', ''), contentWithoutFrontmatter.trim(), config, agentName, filename));
            }
            return memories;
        }
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            memories.push(this.createMemory(title, body, config, agentName, filename, contentTags));
        }
        return memories;
    }
    /**
     ...
     */
    createMemory(title, content, config, agentName, sourceFile, additionalTags = []) {
        return {
            id: (0, uuid_1.v4)(),
     ...
     */
    async appendToMarkdown(memory, agent, filename) {
        const memoryBankDir = path.join(this.workspacePath, agent.memoryBankPath);
        const filePath = path.join(memoryBankDir, filename);
...
exports.MemoryBankSync = MemoryBankSync;
//# sourceMappingURL=memory-bank-sync.js.map